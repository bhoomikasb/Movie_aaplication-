...
def signup(request):
    if request.method=='POST':
        ...
        if User.objects.filter(username=username).exists():
            return render(request,"./pmax/signup.html",{"error":"username already exists"})
...
def movie_detail(request,movie_id):
  ...
  if request.method=='POST':
     ...
     Review.objects.create(movie=movie,user=user, comment=comment,rating=rating)
...
def book_show(request,show_id):
  ...
  if request.method=='POST':
     ...
     if not selected_seats:
        return render(request,'./pmax/seat_selection.html',{"show":show,"booked_seats":booked_seats,"seats":seats,"error":"please selected atleast one seat"})
     seat_list=selected_seats.split(",")
     if len(seat_list)>show.available_seats:
        return render(request,'./pmax/seat_selection.html',{"show":show,"booked_seats":booked_seats,"seats":seats,"error":"Not enough seats available"})
     total_price=len(seat_list)*show.price
     user=User.objects.get(id=user_id)
     Booking.objects.create(user=user,show=show,seats=selected_seats,total_price=total_price)
     show.available_seats=show.available_seats-len(seat_list)
     show.save()
     return render(request,"./pmax/sucess.html",{"show":show,"total_price":total_price,"seats":len(seat_list)})
